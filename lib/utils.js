...
/* versions is coming from github hence it is supposed to be sorted already */
exports.findLatestOfMajor = function (versions, majorTarget) {

    return versions.reduce((latestVersionOfMajor, currentVersion) => {

        const [currentMajor] = currentVersion.split('.');
        if (Number(currentMajor) !== majorTarget) {
            return latestVersionOfMajor;
        }
