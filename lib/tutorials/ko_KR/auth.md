...
이 메소드는 기본값으로 사용할 strategy 이름 문자열 또는 route 처리기의 [auth options](#route-optionsuration)과 같은 형식의 객체를 하나의 인자를 받습니다.
...
`options.auth` 인자로 route의 인증을 설정할 수 있습니다. `false`로 설정되면 route에 인증이 사용되지 않습니다.
