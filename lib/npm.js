...
exports.methods.push({
    name: 'npm.package',
    method: async (packageName) => await Utils.download(`https://registry.npmjs.org/${packageName}`, { json: true, rejectUnauthorized: false }),
    options: {
        cache: {
            expiresIn: Utils.fifteenMinutes,
            generateTimeout: Utils.oneMinute
        },
        generateKey: (packageName) => `npm.package.${packageName}`
    }
});
